...
export default function AdminPage() {
      ...
      if (user) {
        ...
        if (profile?.role !== 'admin') {
          await supabase
            ...
            .update({ role: 'admin' })
            .eq('id', user.id)
          toast.info(t('admin.roleGranted'))
        }
      }
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (error) {
        ...
        toast.error(t('admin.saveError') + ': ' + error.message)
      } else {
        toast.success(editingRoom ? t('admin.updateSuccess') : t('admin.addSuccess'))
        fetchData()
  ...
  const handleForceRelease = async (room: any) => {
    ...
    try {
      ...
      if (updateError) {
        toast.error(t('admin.forceReleaseError'))
        return
      }
  ...
  const exportRoomsCSV = () => {
    exportToCSV(
      filteredRooms.map(r => ({
        ...
        status: r.is_out_of_service ? t('admin.outOfService') : (r.is_occupied ? t('admin.occupied') : t('admin.free')),
      ...
      <div className="min-h-screen relative overflow-hidden bg-[#F5F7FA] flex items-center justify-center p-4">
        ...
        <Card className="relative w-full max-w-md rounded-2xl border-[#E7E5EC] shadow-lg">
          <CardHeader>
            ...
            <CardTitle className="text-2xl font-bold" style={{ color: PRIMARY }}>{t('admin.accessRequired')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="admin-code">{t('admin.enterCode')}</Label>
              <Input
                id="admin-code"
                type="password"
  ...
  const statCards = [
    { label: t('admin.totalRooms'), value: stats.totalRooms, icon: Building2, color: '#7C5CFC' },
    { label: t('admin.users'), value: stats.totalUsers, icon: Users, color: '#7C5CFC' },
    { label: t('admin.freeRooms'), value: stats.freeRooms, icon: CircleCheck, color: '#14B8A6' },
    { label: t('admin.occupiedRooms'), value: stats.occupiedRooms, icon: DoorOpen, color: '#FF6F61' },
    { label: t('admin.confidential'), value: stats.confidentialRooms, icon: Lock, color: '#7C5CFC' },
    { label: t('admin.outOfService'), value: stats.outOfServiceRooms, icon: Ban, color: '#6B6B7A' },
  ]
    ...
    <div className="min-h-screen bg-[#F5F7FA]">
      <div className="relative overflow-hidden blueprint-grid-bg border-b border-[#E7E5EC]">
        ...
        <div className="relative w-full py-10 px-4">
          <div className="flex justify-between items-center flex-wrap gap-4">
            <div>
              <span className="text-xs font-semibold tracking-[0.2em] uppercase" style={{ color: PRIMARY }}>{t('admin.adminSpace')}</span>
              <h1 className="font-display text-3xl font-bold text-[#1A1A2E] mt-1">{t('admin.title')}</h1>
            </div>
            <div className="flex gap-2">
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    <DialogTitle>{editingRoom ? t('admin.editRoom') : t('admin.newRoom')}</DialogTitle>
                  </DialogHeader>
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="w-4 h-4"
                      />
                      <Label htmlFor="confidential">{t('admin.confidentialRoom')}</Label>
                    </div>
                    <div className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        ...
                        className="w-4 h-4"
                      />
                      <Label htmlFor="out_of_service">{t('admin.outOfServiceRoom')}</Label>
                    </div>
                    {formData.is_out_of_service === 'true' && (
                      <div>
                        <Label>{t('admin.outOfServiceReason')}</Label>
                        <Input
                    ...
                    <Button onClick={handleSubmit} style={{ backgroundColor: PRIMARY }} className="w-full hover:opacity-90 text-white">
                      {editingRoom ? t('admin.updateRoom') : t('admin.addRoomButton')}
                    </Button>
                  </div>
                </DialogContent>
              </Dialog>
      ...
      <div className="w-full py-8 px-4">
        ...
        <Card className="rounded-2xl border-[#E7E5EC] shadow-sm">
          <CardHeader>
            <CardTitle className="text-[#1A1A2E]">{t('admin.roomManagement')}</CardTitle>
          </CardHeader>
            ...
            <div className="flex flex-wrap items-center gap-3 px-6 pb-4">
              ...
              <div className="flex items-center gap-1 rounded-lg bg-[#F5F7FA] border border-[#E7E5EC] p-1">
                {([
                  ...
                  { key: 'free', label: t('admin.free') },
                  { key: 'occupied', label: t('admin.occupied') },
                  { key: 'confidential', label: t('admin.confidentialLabel') },
                  { key: 'out_of_service', label: t('admin.outOfServiceLabel') },
                ] as const).map(opt => (
                  <button
                    key={opt.key}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t('admin.capacity')}</TableHead>
                  <TableHead>{t('admin.people')}</TableHead>
                  ...
                  <TableRow key={room.id}>
                    ...
                    <TableCell>
                      {room.is_out_of_service ? (
                        ...
                        >
                          {room.is_occupied ? t('admin.occupied') : t('admin.free')}
                        </span>
                      )}
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        {room.is_occupied && (
                          ...
                          >
                            <DoorOpen className="w-3.5 h-3.5 mr-1" /> {t('admin.forceRelease')}
                          </Button>
                        )}
                        <Button variant="outline" size="sm" onClick={() => handleEdit(room)}>
                          <Pencil className="w-3.5 h-3.5 mr-1" /> {t('admin.edit')}
                        </Button>
                        <Button variant="destructive" size="sm" onClick={() => handleDelete(room.id)}>
                          <Trash2 className="w-3.5 h-3.5 mr-1" /> {t('admin.deleteRoom')}
                        </Button>
                      </div>
                    </TableCell>
                  </TableRow>
        ...
        <Card className="mt-8 rounded-2xl border-[#E7E5EC] shadow-sm">
          <CardHeader>
            <CardTitle className="text-[#1A1A2E]">{t('admin.userManagement')}</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            <div className="flex flex-wrap items-center gap-3 px-6 pb-4">
              <div className="relative flex-1 min-w-[200px]">
              ...
              <div className="flex items-center gap-1 rounded-lg bg-[#F5F7FA] border border-[#E7E5EC] p-1">
                {([
                  { key: 'all', label: t('admin.allRoles') },
                  { key: 'admin', label: t('admin.adminRole') },
                  { key: 'user', label: t('admin.userRole') },
                ] as const).map(opt => (
                  <button
                    key={opt.key}
                      ...
                      <div className="flex gap-1">
                        {!user.is_verified ? (
                          ...
                          <Button
                            ...
                            className="text-xs h-8 border-red-300 text-red-600 hover:bg-red-50"
                          >
                            <XCircle className="w-3.5 h-3.5 mr-1" /> {t('admin.deactivateUser')}
                          </Button>
                        )}
                      </div>
                    </TableCell>
              ...
              <TableBody>
                {history.slice(0, 15).map((item) => (
                  <TableRow key={item.id}>
                    <TableCell className="font-medium">{item.rooms?.name || t('admin.unknownRoom')}</TableCell>
                    <TableCell>
                      <span
                        className="px-2 py-1 rounded-full text-xs font-medium text-white"
                        style={{ backgroundColor: item.is_occupied ? '#FF6F61' : '#14B8A6' }}
                      >
                        {item.is_occupied ? t('admin.occupiedAction') : t('admin.freedAction')}
                      </span>
                    </TableCell>
