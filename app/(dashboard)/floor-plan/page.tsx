...
export default async function FloorPlanPage() {
  ...
  const ceo = rooms?.find(r => r.category === 'bureau')
  const reunion = rooms?.find(r => r.category === 'reunion')
  const postes = rooms?.filter(r => r.category === 'poste') || []
  const detente = rooms?.find(r => r.category === 'detente')
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-green-50 rounded-lg p-4 border-2 border-green-300">
          ...
          <div className="grid grid-cols-2 gap-2">
            {postes.map((poste) => (
              <RoomCard key={poste.id} room={poste} small />
            ))}
          </div>
        </div>
      </div>

      {/* Lounge Area - Full width */}
      {detente && (
...
function RoomCard({ room, small = false }: { room: any, small?: boolean }) {
  const statusColors = {
    free: 'bg-green-500',
    occupied: 'bg-red-500',
    maintenance: 'bg-yellow-500'
      ...
      <Card className={`hover:shadow-lg transition-shadow cursor-pointer ${small ? 'p-1' : ''}`}>
        <CardContent className={`${small ? 'p-2' : 'p-4'}`}>
          <div className="flex justify-between items-center">
            ...
            <Badge className={`${statusColors[room.status] || 'bg-gray-500'} text-white text-xs`}>
              {statusLabels[room.status] || room.status}
            </Badge>
          </div>
          {!small && (
            <>
              <p className="text-sm text-gray-500 mt-1">👥 {room.capacity} person{room.capacity > 1 ? 's' : ''}</p>
              {room.equipment && room.equipment.length > 0 && (
                <p className="text-xs text-gray-400 mt-1"> {room.equipment.slice(0, 3).join(' • ')}</p>
              )}
            </>
          )}
        </CardContent>
